...
describe("BusinessController", () => {
  ...
  it("GET /v1/business/:id returns 200 with business + connections", async () => {
    getUseCase.execute.mockResolvedValue(businessWithConnections);
    await request(app.getHttpServer())
      .get(`/v1/business/${BIZ_ID}`)
      .expect(200, { data: businessWithConnections });
  });

  it("GET /v1/business/:id returns 404 when not found", async () => {
  ...
  it("POST /v1/business returns 201 with created business", async () => {
  ...
  it("POST /v1/business returns 400 for invalid accountingProvider", async () => {
    await request(app.getHttpServer())
      .post("/v1/business")
      .send({
        accountId: ACCOUNT_ID,
  ...
  it("POST /v1/business returns 400 for invalid senderEmail", async () => {
    await request(app.getHttpServer())
      .post("/v1/business")
      .send({
        accountId: ACCOUNT_ID,
  ...
  it("PATCH /v1/business/:id returns 200 with updated settings", async () => {
    updateUseCase.execute.mockResolvedValue(businessSettings);
    await request(app.getHttpServer())
      .patch(`/v1/business/${BIZ_ID}`)
      .send({ senderName: "New Name" })
      .expect(200, { data: businessSettings });
  });

  it("PATCH /v1/business/:id returns 404 when not found", async () => {
  ...
  it("PATCH /v1/business/:id returns 400 for invalid senderEmail", async () => {
    await request(app.getHttpServer())
      .patch(`/v1/business/${BIZ_ID}`)
  ...
  it("DELETE /v1/business/:id returns 204", async () => {
    deleteUseCase.execute.mockResolvedValue(undefined);
    await request(app.getHttpServer())
      .delete(`/v1/business/${BIZ_ID}`)
      .expect(204);
  });

  it("DELETE /v1/business/:id returns 404 when not found", async () => {
