...
describe("cloudEventsArraySchema", () => {
  it("parses a single valid event", () => {
    const parsed = cloudEventsArraySchema.parse([validEvent]);
    expect(parsed).toHaveLength(1);
    expect(parsed[0].id).toBe(validEvent.id);
  });

  it("parses multiple events from different realms", () => {
    const parsed = cloudEventsArraySchema.parse([
      validEvent,
  ...
  it("rejects empty array", () => {
    expect(() => cloudEventsArraySchema.parse([])).toThrow();
  });

  it("rejects an object (legacy eventNotifications shape)", () => {
    expect(() =>
      cloudEventsArraySchema.parse({ eventNotifications: [] }),
    ).toThrow();
  });

  it("rejects events with specversion !== '1.0'", () => {
    expect(() =>
      cloudEventsArraySchema.parse([{ ...validEvent, specversion: "0.3" }]),
    ).toThrow();
  });
  ...
  it("rejects events missing intuitaccountid", () => {
    ...
    void _x;
    expect(() => cloudEventsArraySchema.parse([without])).toThrow();
  });
  ...
  it("rejects events with non-RFC3339 time", () => {
    expect(() =>
      cloudEventsArraySchema.parse([{ ...validEvent, time: "yesterday" }]),
    ).toThrow();
  });
});
