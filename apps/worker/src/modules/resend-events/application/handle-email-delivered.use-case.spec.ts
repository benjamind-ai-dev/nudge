import { HandleEmailDeliveredUseCase } from "./handle-email-delivered.use-case";
...
describe("HandleEmailDeliveredUseCase", () => {
  it("updates message status to delivered", async () => {
    const repo = makeRepo();
    const useCase = new HandleEmailDeliveredUseCase(repo);

    await useCase.execute({ externalMessageId: "re_abc" });

    expect(repo.updateStatus).toHaveBeenCalledWith("msg-uuid", "biz-uuid", "delivered");
  });
