...
describe("EnqueueBusinessSyncsUseCase", () => {
  ...
  it("enqueues one 'invoice-sync' child per eligible connection with correct jobId and options", async () => {
    ...
    expect(queue.add).toHaveBeenNthCalledWith(
      ...
      "invoice-sync",
      { connectionId: "conn-2" },
      expect.objectContaining({ jobId: "sync-conn-2" }),
    );
  });

  it("enqueues two jobs when a business has two connected connections with different ids", async () => {
    reader.findAllSyncable.mockResolvedValue([
      conn("conn-qb", "quickbooks", "biz-1"),
      conn("conn-xero", "quickbooks", "biz-1"),
    ]);
    ...
    expect(queue.add).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({ jobId: "sync-conn-qb" }),
    );
    expect(queue.add).toHaveBeenNthCalledWith(
      2,
      "invoice-sync",
      { connectionId: "conn-xero" },
      expect.objectContaining({ jobId: "sync-conn-xero" }),
    );
  });
