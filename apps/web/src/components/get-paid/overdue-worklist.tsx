...
function ExpandedPanel({ row }: { row: OverdueRow }) {
  ...
  return (
    <div className="border-t border-[#C5C6CF] bg-[#F3F3F3] px-6 py-5">
      <div className="grid items-stretch gap-4 md:grid-cols-3">
        {/* Card 1: Invoice Details */}
        <div className="flex flex-col gap-3 rounded-lg border border-[#C5C6CF] bg-white p-4 shadow-sm">
          ...
          {row.paymentLinkUrl && (
            <button
              ...
              className="mt-auto flex w-full items-center justify-center gap-1.5 rounded-md border border-[#C5C6CF] px-3 py-2 text-sm font-medium text-[#1A1C1C] transition-colors hover:bg-[#F3F3F3]"
            >
  ...
  if (status === "none") {
    return (
      <button
        ...
        className="flex items-center gap-1.5 rounded-md bg-[#0B61A1] px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition-colors hover:bg-[#0a5690]"
      >
  ...
  if (status === "active") {
    return (
      <button
        ...
        className="flex items-center gap-1.5 rounded-md border border-[#C5C6CF] px-3 py-1.5 text-xs font-semibold text-[#45464E] transition-colors hover:bg-white"
      >
      ...
      <div className="flex flex-col md:hidden">
        {rows.map((row) => (
          <div
            key={row.id}
            className={cn(
              "border-b border-[#C5C6CF] last:border-b-0",
              row.isSevere && "bg-[#BA1A1A]/[0.04]",
            )}
          >
            <button
              type="button"
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div className="flex flex-col gap-0.5">
                  <span className="font-semibold text-[#041534]">{row.customerName}</span>
              ...
              <div className="mt-3 flex items-end justify-between gap-2">
                <div className="flex flex-col gap-0.5">
                  ...
                  <span className="font-bold tabular-nums text-[#1A1C1C]">
                    {row.balanceDue}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
              ...
              <Fragment key={row.id}>
                <tr
                  ...
                  className={cn(
                    ...
                    row.isSevere && "bg-[#BA1A1A]/[0.04] hover:bg-[#BA1A1A]/[0.07]",
                    expandedId === row.id && !row.isSevere && "bg-[#F9F9F9]",
                  )}
                >
                  {/* Expand chevron */}
                  ...
                  {/* Days Overdue — "{n} days" with aging dot */}
                  <td className="px-4 py-4">
                  ...
                  {/* Amount — dark text, bold, no decimals */}
                  <td className="px-4 py-4 text-right font-bold tabular-nums text-[#1A1C1C]">
                    {row.balanceDue}
                  </td>
