...
describe("useGetPaidViewModel", () => {
  ...
  // 2. Formats cents correctly
  it("formats balanceDue with formatCents", () => {
    mockInvoices = [makeInvoice({ balanceDueCents: 1_234_56 })];
    const { result } = renderHook(() => useGetPaidViewModel(), { wrapper });
    // $1,234.56
    expect(result.current.rows[0].balanceDue).toBe("$1,234.56");
  });

  // 3. Status derivation — none (null sequenceRun)
  ...
  // 6. Unexpected sequenceRun status falls back to 'none'
  ...
  // 10. openDialog populates dialog fields
  ...
  // 12. handleStartFollowUp calls mutation with correct args
  ...
  // 13. already_running closes dialog and sets alreadyRunning flag
  ...
  // 14. mutation error surfaces in startError
  ...
  // 15. toggleExpand opens and closes a row
  ...
  // 16. agingDotColor assigned per days overdue bucket
