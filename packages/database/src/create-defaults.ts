...
export const STANDARD_STEPS: StepTemplate[] = [
  {
    ...
    channel: "email",
    subjectTemplate:
      "Friendly reminder: Invoice {{invoice_number}} is due",
    bodyTemplate: `Hi {{contact_name}},

I hope this message finds you well. This is a quick reminder that invoice {{invoice_number}} for {{amount}} was due on {{due_date}}.
...
{{sender_name}}`,
  ...
  },
  {
    ...
    channel: "email",
    subjectTemplate:
      "Following up: Invoice {{invoice_number}} — {{days_overdue}} days past due",
    bodyTemplate: `Hi {{contact_name}},

I wanted to follow up regarding invoice {{invoice_number}} for {{amount}}, which is now {{days_overdue}} days past the due date of {{due_date}}.

We understand things can slip through the cracks, so just a polite nudge to see if we can get this resolved. The outstanding balance is {{balance_due}}.
...
{{sender_name}}`,
  ...
  },
  {
    ...
    subjectTemplate:
      "Action needed: Invoice {{invoice_number}} is {{days_overdue}} days overdue",
    bodyTemplate: `Hi {{contact_name}},

I'm reaching out again regarding the outstanding balance of {{balance_due}} on invoice {{invoice_number}}, originally due on {{due_date}}. This invoice is now {{days_overdue}} days past due.
...
{{sender_name}}`,
  ...
  },
  {
    ...
    channel: "email",
    subjectTemplate:
      "Urgent: Invoice {{invoice_number}} is significantly overdue",
    bodyTemplate: `Hi {{contact_name}},

This is an important notice regarding invoice {{invoice_number}} for {{amount}}. The balance of {{balance_due}} is now {{days_overdue}} days past due, and we have not yet received payment or a response to our previous reminders.

We value our relationship with {{company_name}} and want to avoid any further escalation. Please arrange payment as soon as possible or contact us immediately to discuss a resolution.
...
{{sender_name}}`,
  ...
  },
  {
    ...
    channel: "email",
    subjectTemplate:
      "Final notice: Immediate payment required for Invoice {{invoice_number}}",
    bodyTemplate: `Hi {{contact_name}},

This is a final notice regarding the overdue balance of {{balance_due}} on invoice {{invoice_number}}, which is now {{days_overdue}} days past due.
...
{{sender_name}}`,
  ...
  },
  {
    ...
    channel: "email",
    subjectTemplate:
      "[Internal] Overdue alert: {{company_name}} — Invoice {{invoice_number}}",
...
export const VIP_STEPS: StepTemplate[] = [
  {
    ...
    channel: "email",
    subjectTemplate:
      "Quick note regarding Invoice {{invoice_number}}",
    bodyTemplate: `Hi {{contact_name}},

Thank you for your continued partnership — we truly value working with {{company_name}}.

Just a gentle heads-up that invoice {{invoice_number}} for {{amount}} was due on {{due_date}}. No rush if it's already in process — we just wanted to make sure it's on your radar.
...
{{sender_name}}`,
  ...
  },
  {
    ...
    channel: "email",
    subjectTemplate:
      "Checking in: Invoice {{invoice_number}} from {{due_date}}",
    bodyTemplate: `Hi {{contact_name}},

I hope you're doing well. I'm personally following up on invoice {{invoice_number}} for {{amount}}, which was due on {{due_date}}. The current balance is {{balance_due}}.

I completely understand that timing doesn't always work out perfectly, and I wanted to check in to see if there's anything on your end we should be aware of. If there's a preferred payment timeline that works better for {{company_name}}, I'm happy to discuss.
...
{{sender_name}}`,
  ...
  },
  {
    ...
    channel: "email_and_sms",
    subjectTemplate:
      "Following up on your outstanding balance — Invoice {{invoice_number}}",
    bodyTemplate: `Hi {{contact_name}},

I wanted to circle back regarding invoice {{invoice_number}}, which has an outstanding balance of {{balance_due}} and is now {{days_overdue}} days past the due date.

Given our strong relationship with {{company_name}}, I want to make sure there isn't an issue we can help with. Whether it's a billing question, a dispute on the invoice, or simply a matter of timing, we're here to work with you.
...
{{sender_name}}`,
  ...
  },
  {
    ...
    channel: "email",
    subjectTemplate:
      "Wanted to connect about Invoice {{invoice_number}} — {{days_overdue}} days outstanding",
    bodyTemplate: `Hi {{contact_name}},

I'm reaching out once more about invoice {{invoice_number}} with an outstanding balance of {{balance_due}}. It's been {{days_overdue}} days since the original due date, and I want to make sure everything is okay on your end.
...
{{sender_name}}`,
  ...
  },
  {
    ...
    channel: "email",
    subjectTemplate:
      "Important: Resolution needed for Invoice {{invoice_number}}",
    bodyTemplate: `Hi {{contact_name}},

I'm writing with some urgency regarding the outstanding balance of {{balance_due}} on invoice {{invoice_number}}, now {{days_overdue}} days past due.

We have great respect for {{company_name}} and the relationship we've built together, which is why I'd like to resolve this matter directly rather than involve any formal process. However, I do need to hear from you soon so we can agree on a path forward.
...
{{sender_name}}`,
  ...
  },
  {
    ...
    channel: "email",
    subjectTemplate:
      "[Internal] VIP account alert: {{company_name}} — Invoice {{invoice_number}}",
    bodyTemplate: `Hi {{sender_name}},
...
1. Reach out to {{contact_name}} with a personal phone call
